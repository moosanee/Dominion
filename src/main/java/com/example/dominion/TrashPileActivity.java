...
public class TrashPileActivity extends AppCompatActivity {

    ArrayList<CardData> trashPileData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_trash_pile);

        trashPileData = (ArrayList<CardData>) getIntent().getSerializableExtra("trashPileListKey");

        LinearLayout trashPileLayout = (LinearLayout) findViewById(R.id.trash_pile_layout);
        LinearLayout trashCardLayout = (LinearLayout) findViewById(R.id.trash_card_layout);

        if (trashPileData.size() == 0){
            ...
            trashCardLayout.addView(noCardsText);
            cardChosen = false;
        }
        else {
           String drawableString = "";
            for (int i = 0; i < trashPileData.size(); i++){
                String cardName = trashPileData.get(i).getCardName();
                ...
                trashPileLayout.addView(newCardLayout);
                // create card image
                ...
                ButtonInfo biTag = new ButtonInfo(i,trashPileData.get(i).getCardName());
    ...
    void setButtonListeners(){
        ...
        Button exitButton = findViewById(R.id.exit_trash);
